...
# chrome (amd64 only)
docker build . --build-arg STAGE_DEPLOY=apiv1/remote-desktop -f Dockerfile.chrome --target chrome -t apiv1/remote-desktop:chrome --push
docker buildx build . --build-arg STAGE_DEPLOY=apiv1/remote-desktop -f Dockerfile.chrome --target chrome --platform linux/amd64 --pull --push -t apiv1/remote-desktop:chrome
```